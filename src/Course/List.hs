...
map ::
  ...
  -> List a
  -> List b
map f = foldRight (\v cum -> f v :. cum) Nil
...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\v cum -> if f v then v :. cum else cum) Nil
...
seqOptional xs = foldRight (\v cum -> bindOptional (\m -> mapOptional (\c -> m :. c) cum ) v) (Full Nil) xs
...
reverse ::
  List a
  -> List a
reverse = foldLeft (\cum x -> x :. cum) Nil
