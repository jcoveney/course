...
instance Functor ParseResult where
  _ <$> (ErrorResult pe) = ErrorResult pe
...
character =
  let
    proc Nil = ErrorResult UnexpectedEof
    proc (h:.t) =  Result t h
  in P proc
...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f pa = P (\i -> f <$> parse pa i)
...
bindParser f pa =
  let
    ...
    next (ErrorResult r) = ErrorResult r
  in P $ next . (parse pa)
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
pl ||| pr = P (\i -> case parse pl i of r@(ErrorResult _) -> parse pr i; els -> els)
...
list1 ::
  Parser a
  -> Parser (List a)
list1 pa = P (\i -> case parse pa i of (ErrorResult r) -> ErrorResult r; Result i' a -> (a :.) <$> parse (list pa) i')
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = character `flbindParser` (\c -> if f c then valueParser c else unexpectedCharParser c)
...
natural ::
  Parser Int
natural = (read <$> list1 digit) `flbindParser` (\opt -> case opt of Empty -> failed; Full d -> valueParser d)
...
conc :: Parser a -> Parser (List a) -> Parser (List a)
conc pa pb = pa `flbindParser` (\r -> (r:.) <$> pb)
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser (h:.Nil) = (:.Nil) <$> h
sequenceParser (h:.t) = h `conc` sequenceParser t
...
ageParser ::
  Parser Int
ageParser = natural `flbindParser` (\c -> if c > 0 then valueParser c else failed)
...
firstNameParser ::
  Parser Chars
firstNameParser = upper `conc` list lower
...
surnameParser ::
  Parser Chars
surnameParser = firstNameParser `flbindParser` (\w -> if length w >= 5 then valueParser w else failed)
...
phoneParser ::
  Parser Chars
phoneParser = digit `conc` phoneBodyParser `flbindParser` (\x -> P $ \i -> const x <$> parse (is '#') i)
...
personParser ::
  Parser Person
personParser = lift2 (\f p -> f p) (lift4 (Person) ageParser (spaces1 >>> firstNameParser) (spaces1 >>> surnameParser) (spaces1 >>> smokerParser)) (spaces1 >>> phoneParser)
